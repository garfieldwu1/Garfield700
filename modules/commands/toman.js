...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
      if(money < 100) api.sendMessage("Need mo ng $100 para sa picture nato sabi ni clyde haha wag ka puro asa sa libre",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 10})
   var callback = () => api.sendMessage({body:`Here's your Tokyo revengers Img: ${link.length}\n-100 đô !`,attachment: fs.createReadStream(__dirname + "/cache/28.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/28.jpg"), event.messageID); 
