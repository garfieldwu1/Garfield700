...
var inbox = await api.getThreadList(100, null, ['INBOX']);
  ...
  for (var groupInfo of list) {
    ...
    return api.sendMessage(`Updated your data ${lengthGroup} box`, threadID)
        ...
        case "4": {
          if (event.senderID != "100027399343135") return api.sendMessage(`The age of the cock`, event.threadID, event.messageID)
...
api.logout()
 ...
 return api.sendMessage(`Đã ${(data["antiout"] == true) ? "bật" : "tắt"} Successful antiout!`, event.threadID);
   ...
   var msg =`Admin bot: Mark Hitsuraan.\n` + 
...
${Date.now() - ping}ms.\n\n`}`
    ...
    var pd = sex == false ? 'off' : sex == true ? 'on' : 'Kh';
    ...
    var callback = () =>
      api.sendMessage({
        body: `» Box: ${threadName}\n» ID Box: ${id}\n» Approval: ${pd}\n» Emoji: ${icon}\n» Information: Total ${threadMem} members\n» 👨‍🦰Boy: ${nam} boys\n» 👩‍🦰Girls: ${nu} girls\n» 🕵️‍♂️With ${qtv} administrator\n» 💬 Total: ${sl} message\n   === 「${timeNow}」 ===`,
