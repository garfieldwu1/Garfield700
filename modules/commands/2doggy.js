...
module.exports.onLoad = async() => {
  ...
  const path = resolve(__dirname, 'cache/canvas', 'doggy2.png');
...
async function makeImage({ one, two }) {
  ...
  let doggy_img = await jimp.read(__root + "/doggy2.png");
...
module.exports.run = async function ({ event, api, args }) {
  ...
  if (!mention) return api.sendMessage("Please tag 1 person", threadID, messageID);
      ...
      return makeImage({ one, two }).then(path => api.sendMessage({ body: "hey hey:)) " + tag + '',
          mentions: [{
        tag: tag,
        id: mention
      }],
